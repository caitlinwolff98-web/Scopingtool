...
' ============================================================================
' MODULE 1: MAIN CONTROLLER & USER INTERFACE
' ISA 600 Revised Component Scoping Tool - Bidvest Group
' Version: 6.0 - Complete Overhaul
' ============================================================================
' PURPOSE: Main entry point and workflow orchestration for the scoping tool
' DESCRIPTION: Manages the entire process from workbook selection through
'              final output generation with user guidance at each step
' ============================================================================
...
Public Sub StartBidvestScopingTool()
    ...
    result = MsgBox( _
        ...
        vbOKCancel + vbInformation, "Bidvest Scoping Tool v6.0")
    ...
    MsgBox _
        ...
        "✓ Full Input Table & Percentages" & vbCrLf & _
        "✓ Discontinued, Journals, Consol Tables" & vbCrLf & _
        "✓ Division-Segment Mapping" & vbCrLf & _
        "✓ Interactive Dashboard (5 views)" & vbCrLf & _
        "✓ Manual Scoping Interface" & vbCrLf & _
        "✓ Power BI-Ready Tables" & vbCrLf & _
        "✓ Scoping Summary & Reports" & vbCrLf & vbCrLf & _
        "Next Steps:" & vbCrLf & _
        "1. Review Dashboard sheet for overview" & vbCrLf & _
        "2. Use Manual Scoping interface to adjust" & vbCrLf & _
        "3. Import to Power BI for advanced analysis" & vbCrLf & vbCrLf & _
        "See COMPREHENSIVE_IMPLEMENTATION_GUIDE.md for details.", _
        vbInformation, "Process Complete - Bidvest Scoping Tool v6.0"
...
Private Function SelectCurrencyType() As Boolean
    ...
    If g_UseConsolidationCurrency Then
        confirmMsg = confirmMsg & "✓ Consolidation Currency" & vbCrLf & vbCrLf & _
                     "The tool will process columns identified as" & vbCrLf & _
                     "'Consolidation' or 'Consolidable' in Row 6."
    Else
        confirmMsg = confirmMsg & "✓ Entity Currency" & vbCrLf & vbCrLf & _
                     "The tool will process columns identified as" & vbCrLf & _
...
Private Sub CreateOutputWorkbook()
    ...
    With g_OutputWorkbook.Worksheets("ReadMe")
        ...
        End If
        .Range("A6").Value = "Tool Version: 6.0 - Complete Overhaul"
        ...
        Dim row As Long
        row = 9
        .Cells(row, 1).Value = "✓ Full Input Table (amounts)": row = row + 1
        .Cells(row, 1).Value = "✓ Full Input Percentage Table": row = row + 1
        .Cells(row, 1).Value = "✓ Discontinued, Journals, Consol Tables": row = row + 1
        .Cells(row, 1).Value = "✓ FSLi Key Table (reference)": row = row + 1
        .Cells(row, 1).Value = "✓ Pack Number Company Table (reference)": row = row + 1
        .Cells(row, 1).Value = "✓ Division-Segment Mapping (if applicable)": row = row + 1
        .Cells(row, 1).Value = "✓ Interactive Dashboard (multiple views)": row = row + 1
        .Cells(row, 1).Value = "✓ Manual Scoping Interface": row = row + 1
        .Cells(row, 1).Value = "✓ Power BI-Ready Tables": row = row + 1
...
Private Sub ExtractAndGenerateTables()
    ...
    ' Uses Mod3_DataExtraction and Mod3_TableGeneration
    '------------------------------------------------------------------------
